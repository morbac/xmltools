...
namespace StringXml {
    ...
    std::string XmlFormater::linearize() {
        std::string str(this->srcText);

        std::string::size_type curpos = 0, nexwchar_t;
        bool enableInsert = false;

        while ((curpos = str.find_first_of(this->params.eolChars, curpos)) != std::string::npos) {
            nexwchar_t = str.find_first_not_of(this->params.eolChars, curpos);
    ...
    std::string XmlFormater::prettyPrint(bool autoindenttext, bool addlinebreaks, bool indentattributes, bool indentonly) {
        ...
        while (curpos < strlen && (curpos = str.find_first_of("<>\"'\n", curpos)) != std::string::npos) {
            ...
            case '>': {
                if (in_tag) {
                    in_tag = false;
                    if (this->params.autoCloseTags && !str.compare(curpos + 1, 3 + tagname.length(), "</" + tagname + ">")) {
    ...
    std::string XmlFormater::prettyPrint() {
        return this->prettyPrint(false, true, false);
    }

    std::string XmlFormater::prettyPrintAttr() {
        return this->prettyPrint(false, true, true);
    }

    std::string XmlFormater::prettyPrintIndent() {
        return this->prettyPrint(false, true, false, true);
    }
